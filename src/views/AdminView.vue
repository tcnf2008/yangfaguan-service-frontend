      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.username === user.username">当前账号信息</span>
            <el-button type="primary" @click="edit(scope.row)"
              v-if="scope.row.username !== user.username">编辑</el-button>
            <el-popconfirm title="确定删除吗？" @confirm="del(scope.row.id)">
              <el-button slot="reference" type="danger" style="margin-left: 5px"
                v-if="scope.row.username !== user.username">删除</el-button>
            </el-popconfirm>
          </template>
        </el-table-column>
      </el-table>
